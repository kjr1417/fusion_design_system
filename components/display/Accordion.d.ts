...
/**
 * Collapsible sections, based on the Salt accordion. Every section has a
 * hairline border on its TOP edge only (no left/right/bottom, no outer box) and
 * a 37px-tall header (chevron-down when collapsed, chevron-up when expanded,
 * H4 label). `boxed` vs `inline` no longer changes the border treatment — kept
 * for backwards compatibility. Expanded content sits spacing-100 below the
 * header and spacing-150 above the next section, indented spacing-200 plus
 * the icon width to align with the label. Collapsed sections in a stack sit
