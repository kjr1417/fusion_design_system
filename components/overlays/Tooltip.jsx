...
const STATUS = {
  ...
  plain: { bg: "var(--salt-container-primary-background)", border: "var(--salt-color-gray-200)" },
};
...
export function Tooltip({ content, placement = "top", status = "info", maxWidth = 220, style, className, children }) {
    ...
    <span className={className} style={{ position: "relative", display: "inline-flex", ...style }} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
      ...
      {open && (
        <span role="tooltip" style={{
          position: "absolute", zIndex: 10, width: "max-content", maxWidth,
          bottom: placement === "top" ? "calc(100% + 8px)" : "auto",
          top: placement === "bottom" ? "calc(100% + 8px)" : "auto",
          left: "50%", transform: "translateX(-50%)",
          display: "flex", alignItems: "flex-start", gap: glyph ? "var(--salt-spacing-100)" : 0,
          ...
          fontFamily: "var(--salt-text-fontFamily)", boxSizing: "border-box",
        }}>
          {glyph && <svg className="saltIcon" viewBox="0 0 12 12" width="12" height="12" aria-hidden="true" style={{ fill: s.icon, flex: "none" }}><path d={glyph} /></svg>}
