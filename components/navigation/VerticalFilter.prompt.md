...
Layout top to bottom: an optional header row (`title`, default "Filters"; a transparent "Clear all" text button; an icon button that expands/collapses every accordion at once), an optional top `comboBox` field (its own label, no helper text), then the accordion groups — first one open by default (`defaultOpen`). 16px separates the title from the comboBox, 24px separates the last of (title/comboBox) from the first accordion.
